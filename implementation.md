...
The library will support a core set of data types that are common in JSON and JavaScript environments. For a detailed list of supported data types, their psuedo-code, and implementation details, please see the [types.md](./types.md) file.
...
- **String Literals:** Enclosed in single quotes (`'`). Double quotes (`"`) are not supported for defining strings, but are allowed within a single-quoted string. A single quote character can be included in a string by escaping it with a backslash (`\\'`). Backticks (`) define identifiers and are not for strings, but are allowed inside them.
