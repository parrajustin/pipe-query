...
export class UUID {
    /** @param _bytes - The 16-byte byte array representation. */
    private constructor(private readonly _bytes: Readonly<Uint8Array>) {}
     ...
     */
    public static ofInner(bytes: Readonly<Uint8Array>): UUID {
        if (bytes.length !== 16) {
    ...
    public getVariant(): "VAR_0" | "VAR_10" | "VAR_110" | "VAR_RESERVED" | "NIL" | "MAX" {
        ...
        if (n < 0) {
            ...
            return this._bytes.every((e) => e === 0) ? "NIL" : "VAR_0";
        } else if (n <= 0b1011) {
            return "VAR_10";
        } else if (n <= 0b1101) {
            return "VAR_110";
        } else if (n <= 0b1111) {
            return this._bytes.every((e) => e === 0xff) ? "MAX" : "VAR_RESERVED";
        } else {
            throw new Error("unreachable");
        }
    }
    ...
    public clone(): UUID {
        return new UUID(this._bytes.slice(0));
    }
...
export class V7Generator {
    ...
    private readonly _random: { nextUint32(): number };
    ...
    constructor(randomNumberGenerator?: {
        ...
        this._random = randomNumberGenerator ?? getDefaultRandom();
    }
     ...
     */
    public generateOrAbort(): UUID | undefined {
     ...
     */
    public generateOrResetCore(unixTsMs: number, rollbackAllowance: number): UUID {
     ...
     * @throws RangeError if `unixTsMs` is not a 48-bit positive integer.
     */
    public generateOrAbortCore(unixTsMs: number, rollbackAllowance: number): UUID | undefined {
        ...
        if (unixTsMs > this._timestamp) {
            this._timestamp = unixTsMs;
            this.resetCounter();
        } else if (unixTsMs + rollbackAllowance >= this._timestamp) {
