# Internal Knowledge Artifact: Grammar-Well

This document provides a summary of the Grammar-Well tool, which is used in this project.

## Introduction to Grammar-Well
...
Grammar-Well's grammar definition is inspired by EBNF (Extended Backus-Naur Form) and provides additional syntax for output generation.
...
#### Terminals
- **Literals**: Double-quoted strings (e.g., "Hello").
- **Regular Expressions**: Defined using JavaScript's regex syntax within `r:{...}`.
- **Token Tags**: References to lexer-defined tokens, wrapped in angled brackets (e.g., `<ws>`).

### Sub-Expressions

Symbols can be grouped using parentheses `()` to create inline sub-expressions.
...
Post-processors are used to transform or evaluate the values matched by a production rule. They can be JavaScript expressions, function bodies, or interpolated functions from an `on:import` block.

## Lexer

Grammar-Well includes a powerful, optional, stateful lexer. The lexer is defined within a `lexer` block and is composed of named states.

### Lexer States

A lexer state is a set of rules that define how to tokenize the input. The lexer can move between these states.

### Directives

- `when <matcher>`: Matches and consumes input.
- `before <matcher>`: Matches but does not consume input.
- `skip <matcher>`: Matches and consumes input, but the matched text is ignored.
- `goto <state_name>`: Transitions to a new state.
- `pop: <number>`: Pops states from the stack.
- `tag <tag_name>`: Applies a tag to the matched token.

### Spans

Spans are a special type of lexer state for contexts with a start and end delimiter, like strings or comments.

## Imports

Grammar-Well supports importing external grammar and lexer rules for modularity.
...
### Grammar-Well v2 Grammar (`v2.well`)
This is the grammar for Grammar-Well itself.
